  ...
  switch (e) {
    case UIElement::INFO:
      if (fastbootd_logo_enabled_)
        gr_color(0xff, 0xea, 0x80, 255);
      else
        gr_color(0xa8, 0xf4, 0xfc, 255);
      break;
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : nyanpasu_logo_;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    nyanpasu_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    nyanpasu_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(nyanpasu_logo_.get());
      const static int logo_height = gr_get_height(nyanpasu_logo_.get());
